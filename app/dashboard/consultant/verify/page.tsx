'use client'
// FILE: app/dashboard/consultant/verify/page.tsx
// NOTE: This page intentionally uses <Navbar> + <Footer> — it's a standalone
// full-screen verification flow, not nested inside the dashboard sidebar layout.
...
function Toast({ message, type }: { message: string; type: 'success' | 'error' }) {
  return (
    <div className={`fixed top-6 right-6 z-50 flex items-center gap-3 px-5 py-3.5 rounded-2xl shadow-xl text-white text-sm font-medium ${
  ...
  return (
    <div className="text-center py-12 px-6 max-w-lg mx-auto">
      ...
      <p className="text-gray-500 leading-relaxed mb-8">
        Your OEP license and SECP registration have been confirmed. The official{' '}
        <span className="font-semibold text-[#1B3060]">Verified badge</span> is now live on your
        public profile and all your service listings.
      </p>
...
function RejectedState({ onResubmit }: { onResubmit: () => void }) {
  return (
    <div className="text-center py-12 px-6 max-w-lg mx-auto">
      ...
      <p className="text-gray-500 leading-relaxed mb-6">
        We could not verify your credentials with the details provided. This may be due to an
        incorrect license number, title mismatch, or registration date error.
      </p>
      ...
      <div className="flex flex-wrap gap-3 justify-center">
        <button onClick={onResubmit}
          className="inline-flex items-center gap-2 bg-[#1B3060] text-white font-semibold px-7 py-3.5 rounded-xl hover:bg-[#243d7a] transition-colors text-sm">
          <FileText size={17} /> Resubmit Credentials
        </button>
        <a href="mailto:visagate.pk@gmail.com"
          className="inline-flex items-center gap-2 bg-gray-100 text-gray-700 font-semibold px-6 py-3.5 rounded-xl hover:bg-gray-200 transition-colors text-sm">
          Contact Support
        </a>
      </div>
...
// Verification form
// ─────────────────────────────────────────────────────────────────────────────
function VerificationForm({ profileId, onSubmitted }: { profileId: string; onSubmitted: () => void }) {
  ...
  async function handleSubmit() {
    ...
    const { error } = await supabase
      ...
      .eq('id', profileId)  // profileId = profiles.id (PK) — correct ✅

    if (error) { showToast('Submission failed. Please try again.', 'error'); setSaving(false); return }

    showToast('Credentials submitted successfully!', 'success')
    setTimeout(() => onSubmitted(), 1200)
  }

  const fieldClass = "w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#1B3060]/20 focus:border-[#1B3060] placeholder:text-gray-300 hover:border-gray-300 transition-all"
        ...
        <div className="bg-blue-50 border border-blue-100 rounded-2xl p-5 flex gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-sm text-blue-700 leading-relaxed">
              Submit your OEP license details and SECP registration date. Our team manually
              cross-checks these with official records and adds your{' '}
              <span className="font-semibold">Verified badge</span> within 24–48 hours.
            </p>
          </div>
        </div>
          ...
          <div className="px-7 py-7 space-y-6">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" value={oepNumber} onChange={e => setOepNumber(e.target.value)}
                  placeholder="e.g. OEP-12345" className={fieldClass} />
              </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" value={oepTitle} onChange={e => setOepTitle(e.target.value)}
                  placeholder="e.g. Immigration Consultant" className={fieldClass} />
              </div>
              <p className="text-xs text-gray-400 mt-1.5 ml-1">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="date" value={secpDate} onChange={e => setSecpDate(e.target.value)}
                  max={new Date().toISOString().split('T')[0]} className={fieldClass} />
              </div>
              <p className="text-xs text-gray-400 mt-1.5 ml-1">
              ...
              <div className="grid sm:grid-cols-2 gap-2.5">
                {[
                  ...
                  <div key={i} className="flex items-center gap-2">
                    <BadgeCheck size={15} className="text-[#C9A227] shrink-0" />
            ...
            <p className="text-xs text-gray-400 leading-relaxed">
              By submitting, you confirm that all information is accurate and matches official
              government records. False credentials may result in permanent account suspension.
            </p>
...
export default function ConsultantVerifyPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data, error } = await supabase
        ...
        // [FIX] was .eq('id', user.id) — profiles FK is user_id not id
        .eq('user_id', user.id)
        .single()

      if (error || !data) { router.push('/login'); return }
      if (data.role !== 'consultant') { router.push('/dashboard/seeker'); return }
  ...
  async function handleSubmitted() {
    ...
    const { data } = await supabase
      .from('profiles')
      .select('id, full_name, verification_status, oep_license_number, oep_license_title, secp_registration_date, role')
      .eq('id', profile.id)  // profile.id = profiles PK — correct ✅
      .single()
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="bg-[#1B3060] pt-16 pb-20">
          <div className="max-w-3xl mx-auto px-4 sm:px-6">
            ...
            <div className="flex items-start gap-4">
              <div className="w-12 h-12 rounded-2xl bg-[#C9A227]/20 border border-[#C9A227]/30 flex items-center justify-center shrink-0 mt-0.5">
            ...
            <div className="mt-6">
              {status === 'unverified' && (
                <span className="inline-flex items-center gap-2 bg-gray-500/20 border border-gray-400/20 text-gray-300 text-xs font-semibold px-4 py-2 rounded-full">
                  <div className="w-1.5 h-1.5 rounded-full bg-gray-400" /> Not Verified
                </span>
              )}
              {status === 'pending_verification' && (
                <span className="inline-flex items-center gap-2 bg-amber-500/20 border border-amber-400/20 text-amber-300 text-xs font-semibold px-4 py-2 rounded-full">
                  <div className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse" /> Pending Review
                </span>
              )}
              {status === 'verified' && (
                <span className="inline-flex items-center gap-2 bg-green-500/20 border border-green-400/20 text-green-300 text-xs font-semibold px-4 py-2 rounded-full">
                  <div className="w-1.5 h-1.5 rounded-full bg-green-400" /> Verified
                </span>
              )}
              {status === 'rejected' && (
                <span className="inline-flex items-center gap-2 bg-red-500/20 border border-red-400/20 text-red-300 text-xs font-semibold px-4 py-2 rounded-full">
                  <div className="w-1.5 h-1.5 rounded-full bg-red-400" /> Not Approved
                </span>
              )}
            </div>
          </div>
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 -mt-10 pb-24">
                ...
                <div key={i} className="flex items-center flex-1 last:flex-none">
                  <div className="flex flex-col items-center gap-1.5">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${
                      step.done ? 'bg-[#1B3060] text-white'
                        : status === 'rejected' && i === 2 ? 'bg-red-100 text-red-500 border-2 border-red-200'
          ...
          {/* State-based content */}
          {showForm && <VerificationForm profileId={profile!.id} onSubmitted={handleSubmitted} />}
