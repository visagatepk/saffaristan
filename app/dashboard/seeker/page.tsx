...
function StatCard({ label, value, sub, color, spark, featured, trend }: StatCardProps) {
  return (
...
export default function SeekerOverview() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Parallel fetches — seeker_id = auth user id (FK to auth.users, not profiles)
      const [
        { data: saved },
        { count: apptCount },
        { data: appts },
        ...
        // Saved consultants (seeker_id = profile.id)
        supabase
        ...
        // Total appointment count — seeker_id = auth user id
        supabase
          .from('appointments')
          .select('*', { count: 'exact', head: true })
          .eq('seeker_id', user.id),

        // Recent appointments — flat columns, no joins needed (denormalized)
        supabase
          .from('appointments')
      ...
      if (appts && appts.length > 0) {
        ...
        // Next upcoming confirmed appointment (preferred_date in future)
  ...
  return (
    <div className="max-w-[1000px] space-y-5">
      ...
      <div className="bg-[#1B3060] rounded-2xl p-7 relative overflow-hidden">
        ...
        {/* Gold glow top-right */}
        <div
          className="absolute top-0 right-0 w-80 h-80 rounded-full pointer-events-none opacity-[0.18]"
          style={{ background: 'radial-gradient(circle, #C9A227 0%, transparent 70%)', transform: 'translate(30%, -30%)' }}
        />
        {/* Blue glow bottom-left */}
        <div
          className="absolute bottom-0 left-0 w-52 h-52 rounded-full pointer-events-none opacity-10"
        ...
        <div className="relative flex items-center justify-between gap-6 flex-wrap">
          ...
          <div className="flex items-center gap-2.5 shrink-0">
            <Link
              href="/dashboard/seeker/appointments"
      ...
      <div className="flex gap-3.5 flex-wrap">
        <StatCard
          ...
          sub={appointmentCount === 1 ? '1 this week' : `${appointmentCount} total`}
        ...
        <StatCard
          ...
          value={savedCount}
          sub="Last added recently"
          spark={[1, 2, 2, 3, 2, 3, 3, savedCount]}
          color="#C9A227"
        />
        <StatCard
          label="Unread Messages"
          value={unreadCount}
          sub={unreadCount > 0 ? `${unreadCount} new` : 'All caught up'}
          spark={[1, 3, 2, 5, 3, 4, 6, unreadCount]}
          color="#059669"
        />
        <StatCard
          label="Consultants Browsed"
          value="—"
          sub="Start exploring"
          spark={[2, 2, 3, 3, 4, 4, 5, 5]}
          color="#7c3aed"
        />
      </div>
          ...
          {recentAppts.length > 0 ? (
            <div className="divide-y divide-gray-50">
              {recentAppts.map(appt => {
                // appointments table is denormalized — all fields stored directly
                ...
                const date     = appt.preferred_date
                  ...
                  : '—'
                const time     = appt.preferred_time || ''
                return (
                  <div key={appt.id} className="flex items-center gap-3.5 py-3.5">
                    <div className="w-10 h-10 rounded-xl bg-[#EBF0F8] flex items-center justify-center text-lg shrink-0">
                      🌐
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">
                        with {appt.consultant_name || 'Consultant'}
                        {date && ` · ${date}`}
                        {time && ` · ${time}`}
                      </p>
                    </div>
            ...
            <div className="text-center py-10">
              ...
              <p className="font-body text-gray-400 text-sm mb-4">No appointments yet</p>
              <Link
                href="/consultants"
                className="inline-flex items-center gap-1.5 font-heading font-semibold text-xs text-[#1B3060] border border-[#1B3060] px-4 py-2 rounded-xl hover:bg-[#1B3060] hover:text-white transition-colors"
              >
                Find a Consultant
              </Link>
            </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          {nextAppt ? (
            <div
              ...
              style={{ background: 'linear-gradient(135deg, #C9A227 0%, #a8861f 100%)' }}
            >
              <p className="text-[11px] font-bold text-white/70 uppercase tracking-wider mb-2.5">
                Next Appointment
              </p>
              <div className="flex items-center gap-2.5 mb-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/70 mt-0.5">
                    {nextAppt.consultant_name || 'Consultant'}
                  </p>
                </div>
              </div>
              <div className="bg-black/15 rounded-xl px-3.5 py-2.5 mb-3.5">
                ...
                {nextAppt.meeting_link && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-xs text-white/70 mt-1 underline hover:text-white transition-colors block"
                  >
                    Join meeting link
                  </a>
                )}
              ...
              <Link
                href="/dashboard/seeker/appointments"
                className="block w-full text-center py-2.5 rounded-xl bg-white/20 border border-white/30 text-white font-heading font-bold text-[13px] hover:bg-white/30 transition-colors"
              >
                View Details
              </Link>
            </div>
          ) : (
            /* No upcoming appointment — show CTA instead */
            <div
            ...
            >
              <div
                ...
                style={{ background: 'radial-gradient(circle, #C9A227 0%, transparent 70%)', transform: 'translate(30%, -30%)' }}
              />
              <p className="font-heading font-bold text-white text-sm mb-1 relative">
                No upcoming appointment
              </p>
              <p className="text-white/50 text-xs mb-4 relative">
                Book a consultation with a verified consultant today.
              </p>
              <Link
                href="/consultants"
                className="relative inline-flex items-center gap-2 bg-[#C9A227] text-white font-heading font-bold text-sm px-5 py-2.5 rounded-xl hover:bg-[#a8861f] transition-colors"
              >
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {savedConsultants.map(item => {
              ...
              return (
                ...
                >
                  <div className="w-9 h-9 bg-[#1B3060] rounded-xl flex items-center justify-center text-white font-heading font-black text-xs shrink-0">
                    {getInitials(c?.display_name || null)}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-heading font-bold text-[#1B3060] text-[13px] truncate">
                      {c?.display_name}
                    </p>
                  ...
                  <Link
                    ...
                    className="shrink-0 font-heading text-[11px] font-semibold text-[#1B3060] border border-[#1B3060]/25 px-2.5 py-1 rounded-lg hover:bg-[#1B3060] hover:text-white transition-colors"
                  >
                    View
                  </Link>
                </div>
