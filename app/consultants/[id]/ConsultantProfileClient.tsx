...
export default function ConsultantProfileClient({ profile, services, reviews }: Props) {
  ...
  const hasAnyAccreditation =
    profile.is_oep_verified || profile.is_secp_verified ||
    profile.is_beoe_verified || profile.is_fbr_verified
        ...
        <section className="relative">
          ...
          <div className="relative h-44 sm:h-60 md:h-72 lg:h-80 w-full overflow-hidden">
            {profile.cover_image_url ? (
              <Image src={profile.cover_image_url} alt={`${displayName} cover`}
              ...
              <div className="absolute inset-0 bg-gradient-to-br from-[#1B3060] via-[#243d7a] to-[#1B3060]">
                <div className="absolute inset-0 opacity-15"
                  style={{
                    backgroundImage: 'radial-gradient(circle at 20% 30%, rgba(201,162,39,0.4) 1.5px, transparent 1.5px), radial-gradient(circle at 75% 70%, rgba(255,255,255,0.3) 1.5px, transparent 1.5px)',
            ...
            <div className="relative -mt-16 sm:-mt-20 pb-6">
              <div className="flex flex-col lg:flex-row lg:items-end gap-5 lg:gap-6">
                ...
                <div className="relative w-28 h-28 sm:w-36 sm:h-36 lg:w-44 lg:h-44 rounded-full ring-4 ring-white shadow-2xl overflow-hidden bg-white flex-shrink-0">
                  {profile.avatar_url ? (
                    <Image src={profile.avatar_url} alt={displayName} fill
                  ...
                  {profile.is_verified && (
                    <div className="absolute bottom-2 right-2 bg-green-500 rounded-full p-1.5 ring-2 ring-white"
                      aria-label="Verified consultant" title="Verified consultant">
                ...
                <div className="flex-1 flex flex-col xl:flex-row xl:items-end xl:justify-between gap-4 lg:pb-2">
                    ...
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 mt-3 text-sm text-gray-600">
                      {profile.city && (
                        <span className="inline-flex items-center gap-1">
                          <MapPin className="w-4 h-4 text-gray-400" /> {profile.city}
                        </span>
                      )}
                  ...
                  <div className="hidden lg:flex items-center gap-2 flex-shrink-0 flex-wrap">
                    {profile.phone && (
                    ...
                    {profile.phone && (
                      <a href={whatsappUrl(profile.phone)} target="_blank" rel="noopener noreferrer"
                        className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg border border-green-500 bg-green-50 text-green-700 font-medium hover:bg-green-500 hover:text-white transition-colors text-sm">
                        {/* WhatsApp icon */}
              ...
              {hasAnyAccreditation && (
                <div className="flex flex-wrap items-center gap-2 mt-4">
                  ...
                  {profile.is_oep_verified && <AccreditationBadge label="OEP Licensed" />}
                  {profile.is_secp_verified && <AccreditationBadge label="SECP Registered" />}
                  {profile.is_beoe_verified && <AccreditationBadge label="BEOE Approved" />}
                  {profile.is_fbr_verified && <AccreditationBadge label="FBR Compliant" />}
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-2 mt-5 lg:hidden">
                ...
                {profile.phone && (
                  <a href={whatsappUrl(profile.phone)} target="_blank" rel="noopener noreferrer"
                    className="inline-flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-lg border border-green-500 bg-green-50 text-green-700 font-medium text-sm">
                    <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/>
                    </svg>
                    WhatsApp
              ...
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                <div className="border-b border-gray-200 overflow-x-auto scrollbar-hide">
                  <nav className="flex min-w-max" aria-label="Profile tabs">
                    <TabButton active={activeTab === 'services'} onClick={() => setActiveTab('services')} count={services.length}>
                      Services
                    </TabButton>
                    <TabButton active={activeTab === 'reviews'} onClick={() => setActiveTab('reviews')} count={reviews.length}>
                      Reviews
                    </TabButton>
              ...
              <div className="lg:sticky lg:top-24 space-y-4">
                ...
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                    ...
                    <div className="flex items-center gap-3 mt-4 p-3 bg-gradient-to-br from-[#1B3060]/5 to-[#C9A227]/5 rounded-lg border border-gray-100">
                      <div className="text-center">
                        ...
                        <div className="flex items-center justify-center gap-0.5">
                          {[1, 2, 3, 4, 5].map((n) => (
                            <Star key={n} className={`w-3 h-3 ${n <= Math.round(avgRating) ? 'fill-[#C9A227] text-[#C9A227]' : 'text-gray-300'}`} />
                          ))}
                        </div>
                      </div>
                  ...
                  <div className="space-y-2 mt-5">
                    ...
                    {profile.phone && (
                      <a href={whatsappUrl(profile.phone)} target="_blank" rel="noopener noreferrer"
                        className="flex items-center justify-center gap-2 w-full px-4 py-3 rounded-lg border border-green-500 text-green-700 font-semibold hover:bg-green-500 hover:text-white transition-colors">
                        <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/>
                        </svg>
                        WhatsApp
                    ...
                    {profile.website_url && (
                      <a href={profile.website_url.startsWith('http') ? profile.website_url : `https://${profile.website_url}`}
                        ...
                        className="flex items-center justify-center gap-2 w-full px-4 py-3 rounded-lg border border-gray-300 text-gray-700 font-medium hover:bg-gray-50 transition-colors">
                  ...
                  {memberSince && (
                    <p className="text-xs text-gray-500 mt-4 text-center">Member since {memberSince}</p>
                  )}
                </div>
  ...
  return (
    <button type="button" onClick={onClick} role="tab" aria-selected={active}
      ...
      <span className="flex items-center gap-2">
        {children}
        {count != null && count > 0 && (
          <span className={`text-xs px-2 py-0.5 rounded-full ${active ? 'bg-[#1B3060] text-white' : 'bg-gray-100 text-gray-600'}`}>
            {count}
          </span>
        )}
      </span>
...
function ServicesGrid({ services }: { services: Service[] }) {
  if (services.length === 0) {
    return <EmptyState icon={Briefcase} title="No services yet" description="This consultant hasn't published any services. Check back soon or send them a message." />
  }
  return (
  ...
  return (
    <Link href={`/services/${service.id}`}
      ...
      <div className="aspect-[4/3] relative bg-gray-100 overflow-hidden">
        {service.image_url ? (
          <Image src={service.image_url} alt={service.title} fill
      ...
      <div className="flex flex-col flex-1 p-4">
        ...
        {service.destination_country && (
          <p className="text-xs text-gray-500 mt-1.5 inline-flex items-center gap-1">
            <MapPin className="w-3 h-3" /> {service.destination_country}
          </p>
        )}
...
function ReviewsList({ reviews, avgRating, breakdown }: ReviewsListProps) {
  if (reviews.length === 0) {
    return <EmptyState icon={Star} title="No reviews yet" description="Be the first to share your experience after working with this consultant." />
  }
      ...
      <div className="grid sm:grid-cols-2 gap-6 p-4 sm:p-5 bg-gradient-to-br from-[#1B3060]/5 to-[#C9A227]/5 rounded-xl border border-gray-100">
        <div className="flex flex-col items-center justify-center text-center">
          ...
          <div className="flex items-center gap-0.5 mt-2">
            {[1, 2, 3, 4, 5].map((n) => (
              <Star key={n} className={`w-5 h-5 ${n <= Math.round(avgRating) ? 'fill-[#C9A227] text-[#C9A227]' : 'text-gray-300'}`} />
            ))}
          </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-0.5 mt-1">
                    {[1, 2, 3, 4, 5].map((n) => (
                      <Star key={n} className={`w-3.5 h-3.5 ${n <= review.rating ? 'fill-[#C9A227] text-[#C9A227]' : 'text-gray-300'}`} />
                    ))}
                  </div>
                  {review.comment && (
                    <p className="text-sm text-gray-700 mt-2 leading-relaxed whitespace-pre-line">{review.comment}</p>
                  )}
                </div>
              </div>
            </div>
...
function AboutSection({ profile, memberSince }: { profile: Profile; memberSince: string | null }) {
  const hasContent = profile.bio ||
    ...
    profile.office_address || profile.website_url

  if (!hasContent) {
    return <EmptyState icon={Mail} title="No additional information" description="This consultant hasn't filled out their profile yet." />
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {profile.specializations && profile.specializations.length > 0 && (
        <div>
          <h3 className="font-semibold text-gray-900 text-base mb-3 flex items-center gap-2">
            <Tag className="w-4 h-4 text-[#C9A227]" /> Specializations
          </h3>
          <div className="flex flex-wrap gap-2">
            {profile.specializations.map((spec) => (
              <span key={spec} className="px-3 py-1.5 rounded-full bg-[#1B3060]/5 text-[#1B3060] text-sm font-medium border border-[#1B3060]/10">
                {spec}
              </span>
            ))}
          </div>
      ...
      {profile.languages && profile.languages.length > 0 && (
        <div>
          <h3 className="font-semibold text-gray-900 text-base mb-3 flex items-center gap-2">
            <Languages className="w-4 h-4 text-[#C9A227]" /> Languages
          </h3>
          <div className="flex flex-wrap gap-2">
            {profile.languages.map((lang) => (
              <span key={lang} className="px-3 py-1.5 rounded-full bg-gray-100 text-gray-700 text-sm font-medium">{lang}</span>
            ))}
          </div>
        </div>
      )}
      {profile.office_address && (
        <div>
          <h3 className="font-semibold text-gray-900 text-base mb-2 flex items-center gap-2">
            <MapPin className="w-4 h-4 text-[#C9A227]" /> Office Address
          </h3>
      ...
      {profile.website_url && (
        <div>
          <h3 className="font-semibold text-gray-900 text-base mb-2 flex items-center gap-2">
            <Globe className="w-4 h-4 text-[#C9A227]" /> Website
          </h3>
          <a href={profile.website_url.startsWith('http') ? profile.website_url : `https://${profile.website_url}`}
            target="_blank" rel="noopener noreferrer"
            className="inline-flex items-center gap-1.5 text-[#1B3060] hover:text-[#C9A227] transition-colors text-sm font-medium underline underline-offset-2">
            {profile.website_url}
